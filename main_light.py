    ...
    for url in PR_RSS:
        try:
            ...
            for it in root.iterfind('.//item'):
                rows.append({'title':it.findtext('title') or '', 'url':it.findtext('link') or '', 'published_at':it.findtext('pubDate') or ''})
...
def build_news_rows(hours:int=12)->List[Dict[str,Any]]:
    ...
    for it in fetch_benzinga(since_iso):
        ...
        syms=[(t.get('name',t).upper() if isinstance(t,dict) else str(t).upper()) for t in (tickers if isinstance(tickers,list) else [])]
        for sym in syms:
